...
internal sealed class SharePlanHandler : IRequestHandler<SharePlan, Unit>
{
	...
	public async Task<Unit> Handle(SharePlan request, CancellationToken cancellationToken)
	{
		...
		var plan = await _dbContext.Plans.Include(x => x.Shared)
				.ThenInclude(x => x.Follow)
				.SingleOrDefaultAsync(x => x.Id == request.PlanId) ??
			throw new PlanNotFoundException(request.PlanId);
		var follow = await _dbContext.Followers.Include(x => x.Shared)
				.SingleOrDefaultAsync(x => x.FollowerId == shareUser.Id) ??
