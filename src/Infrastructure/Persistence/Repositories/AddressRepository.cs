...
internal sealed class AddressRepository : IAddressRepository
{
	...
	{
		var query = await _dbContext.Addresses.SingleOrDefaultAsync(x => x.Id == id) ?? throw new NotFoundException("Address not found.");
		return query;
	}

	public async Task<Address> GetByPlaceIdAsync(string placeId)
	{
		var query = await _dbContext.Addresses.SingleOrDefaultAsync(x => x.PlaceId == placeId) ?? throw new NotFoundException("Address not found.");
		return query;
	}
