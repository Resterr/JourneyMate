...
internal sealed class GeocodeApiService : IGeocodeApiService
{
	...
	public async Task<AddressAddDto?> GetAddressAsync(string components, string? localityName)
	{
		...
		if (addressReadModel != null)
		{
			if (addressReadModel.Status == "OK")
			{
				...
				if (address != null)
				{
					var isNamevalid = localityName ==
						address.Address_Components.FirstOrDefault(x => x.Types.Any(y => y.Contains("locality") || y.Contains("administrative_area_level_3")))
							?.Long_Name;
					if (isNamevalid)
					{
					...
					Log.Error(components + " " + localityName);
				}
			}
			else
			{
	...
	public async Task<AddressAddDto?> GetAddressFromTerytAsync(string county, string municipality, bool isCity)
	{
		...
		if (addressReadModel != null)
			if (addressReadModel.Status == "OK")
			{
				var address = addressReadModel.Results.FirstOrDefault(x =>
					x.Address_Components.Any(y => y.Types.Contains("locality") || y.Types.Contains("administrative_area_level_3") || y.Types.Contains("administrative_area_level_3")) &&
				...
				if (address != null)
				{
					...
					Log.Error(components + " " + municipality);
				}
				else
				{
					Log.Error(components + " " + municipality);
	...
	private static AddressAddDto MapToDto(AddressReadModel.Result result)
	{
		...
		if (!validationResult.IsValid) throw new AddressNotFoundException();
		...
		var administrativeAreaLevel2 = result.Address_Components.FirstOrDefault(x => x.Types.Contains("administrative_area_level_2")) ??
			throw new ObjectNotFoundException("Administrative area level 2");
		var administrativeAreaLevel1 = result.Address_Components.FirstOrDefault(x => x.Types.Contains("administrative_area_level_1")) ??
			throw new ObjectNotFoundException("Administrative area level 1");
		...
		address.AdministrativeArea = new AdministativeAreaAddDto(new AddressComponent(administrativeAreaLevel1.Short_Name, administrativeAreaLevel1.Long_Name),
			new AddressComponent(administrativeAreaLevel2.Short_Name, administrativeAreaLevel2.Long_Name), new AddressComponent(country.Short_Name, country.Long_Name));
